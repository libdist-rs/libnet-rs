...
unsafe impl<Id, SendMsg> Send for TcpSimpleSender<Id, SendMsg>  where Id: Identifier {}
unsafe impl<Id, SendMsg> Sync for TcpSimpleSender<Id, SendMsg>  where Id: Identifier {}
...
where
    Id: Identifier,
{
    async fn send(&mut self, sender: Id, msg: Bytes) -> Result<(), TcpSimpleSenderError> {
    ...
    async fn broadcast(&mut self, msg: Bytes, peers: &[Id]) -> Result<(), TcpSimpleSenderError> 
    {
        for peer in peers {
